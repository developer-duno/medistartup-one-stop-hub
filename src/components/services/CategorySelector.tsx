...
const CategorySelector: React.FC<CategorySelectorProps> = ({ categories }) => {
  return (
    <div className="mb-8">
      <TabsList className="w-full flex flex-wrap justify-center gap-1.5 md:gap-2 p-1.5 bg-muted/50 rounded-xl h-auto border border-border/50 shadow-sm">
        {categories.map((category) => (
          <TabsTrigger
            key={category.id}
            value={category.id}
            className="px-3 py-2 text-xs md:text-sm md:px-5 md:py-2.5 rounded-lg font-medium transition-all duration-200 flex items-center gap-1.5 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md"
          >
